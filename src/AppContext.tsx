...
export function GameContextProvider(props: { children: ReactNode }) {
  ...
  const addSpiller = (navn: string) => {
    setSpillere((prevState) => [...prevState.filter((it) => it.navn !== navn), { navn: navn, score: defaultScore }]);
  };
  ...
  return (
    <GameContext.Provider
      value={{
        setRunde,
        spillere: spillereMedTotalScore.sort((a, b) => (a.navn === b.navn ? 0 : a.navn < b.navn ? -1 : 1)),
        runde,
        lagreOgStartPåNytt,
        addSpiller,
        updateScore,
